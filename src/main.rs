        ...
        for y in (min.y as usize)..(max.y as usize) {
            ...
            if inside {
                ...
                if z < depth {
                    ...
                    let n0 = *inv_trans_model_matrix * Vec4::from((v0.normal, 0.0));
                    let n1 = *inv_trans_model_matrix * Vec4::from((v1.normal, 0.0));
                    let n2 = *inv_trans_model_matrix * Vec4::from((v2.normal, 0.0));

                    let normal  = ((n0 * v0_clip_space.1 * bary_coords.x
                                        + n1 * v1_clip_space.1 * bary_coords.y
                                        + n2 * v2_clip_space.1 * bary_coords.z).xyz() * correction).normalize();
...
fn draw_model(
    ...
    for mesh in &model.meshes {
        for i in 0..(mesh.indices.len() / 3) {
            ...
            draw_triangle(framebuffer, depth_buffer, &v0, &v1, &v2, mvp, inv_trans_model_matrix, material);
        }
    }
}

fn main() {
    let mut window = Window::new("the fish", 512, 512);
    ...
    while !window.should_close() {
        ...
        draw_model(framebuffer, &mut depth_buffer, &model, &mvp, &inv_trans_model_matrix);
